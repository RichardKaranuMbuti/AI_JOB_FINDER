...
# async def process_job_urls(job_entries, driver=None, max_concurrent=5, checkpoint_freq=10):
#     """
#     Process multiple job URLs concurrently to extract additional details.
#     Uses the same Chrome driver instance.
    
#     Args:
#         job_entries (list): List of job entry dictionaries with 'Job URL' keys
#         driver (webdriver.Chrome, optional): An existing Chrome webdriver
#         max_concurrent (int): Maximum number of concurrent requests
#         checkpoint_freq (int): How often to save checkpoints (every X jobs)
        
#     Returns:
#         list: Updated job entries with additional details
#     """
...
#     completed_jobs = 0
#     checkpoint_filename = os.path.join('checkpoints', f'job_details_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
    
#     # Check if we have a checkpoint to resume from
#     checkpoint_path = os.path.join('checkpoints', 'latest_checkpoint.json')
#     if os.path.exists(checkpoint_path):
#         try:
#             with open(checkpoint_path, 'r') as f:
#                 checkpoint_data = json.load(f)
#                 # Find jobs that have already been processed
#                 processed_ids = {job.get('Job ID') for job in checkpoint_data if job.get('Job ID') != 'N/A'}
#                 # Filter out already processed jobs
#                 job_entries = [job for job in job_entries if job.get('Job ID') not in processed_ids]
#                 # Add already processed jobs to our final list
#                 completed_jobs = len(processed_ids)
#                 print(f"Resuming from checkpoint. {completed_jobs} jobs already processed.")
#         except Exception as e:
#             print(f"Error loading checkpoint: {e}")
    
#     # Create a progress bar
#     progress_bar = tqdm(total=total_jobs, initial=completed_jobs, desc="Processing Jobs")
    
#     # List to hold all completed jobs
#     all_completed_jobs = []
    
#     # If there's a checkpoint, load the already processed jobs
#     if completed_jobs > 0:
#         with open(checkpoint_path, 'r') as f:
#             all_completed_jobs = json.load(f)
    
#     async def fetch_with_semaphore(job_entry, index):
#         async with semaphore:
#             try:
#                 # Use the shared driver if provided, otherwise get job details with a timeout
#                 details = fetch_job_details(job_entry['Job URL'], driver=driver, timeout=config.JOB_DETAIL_TIMEOUT)
                
#                 # Update the job entry with additional details
...
#                 nonlocal all_completed_jobs
#                 all_completed_jobs.append(job_entry)
                
#                 if (index + 1) % checkpoint_freq == 0:
#                     # Save checkpoint
#                     with open(checkpoint_path, 'w') as f:
#                         json.dump(all_completed_jobs, f)
#                     # Also write to CSV incrementally
#                     write_jobs_to_csv(all_completed_jobs, config.JOBS_CSV_FILENAME)
                    
#                 return job_entry
#             except Exception as e:
#                 # Log the error but continue with the next job
#                 progress_bar.write(f"Error processing job {job_entry.get('Job Title')}: {str(e)[:100]}...")
#                 progress_bar.update(1)
#                 return job_entry
    
#     # Create tasks for all job URLs
#     tasks = [fetch_with_semaphore(job, i) for i, job in enumerate(job_entries)]
    
#     # Run the tasks concurrently and gather results
#     new_jobs = await asyncio.gather(*tasks)
    
#     # Close the progress bar
#     progress_bar.close()
    
#     # Combine with already processed jobs if any
#     if len(all_completed_jobs) > len(new_jobs):
#         # This means we loaded some jobs from a checkpoint
